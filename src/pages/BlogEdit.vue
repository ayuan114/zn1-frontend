      ...
      <div class="editor-container">
        <a-form layout="vertical" :model="formState">
          ...
          <div class="form-section">
            ...
            <a-input v-model:value="formState.title" placeholder="请输入文章标题" size="large" />
          </div>
          ...
          <div class="form-section">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <a-select v-model:value="formState" placeholder="请选择文章分类" style="width: 100%">
                  <a-select-option v-for="category in useCategories" :key="category.id" :value="category.name">
                    {{ category.name }}
                  </a-select-option>
                </a-select>
              </div>
              ...
              <div>
                ...
                <a-select v-model:value="formState.tags" mode="multiple" placeholder="请选择文章标签" style="width: 100%"
                  :options="tags" :field-names="{ label: 'name', value: 'id' }"></a-select>
              </div>

              <a-form-item label="标签" name="tags">
                <a-select v-model:value="formState.tags" :options="tagOptions" mode="tags" placeholder="请输入标签"
                  allowClear />
              </a-form-item>
            </div>
          </div>
          ...
          <div class="form-section">
            ...
            <div class="content-editor">
              <div v-for="(block, index) in formState.blocks" :key="index" class="content-block"
                :class="block.type === 'TEXT' ? 'text-block' : 'image-block'">
                <div class="block-counter">{{ index + 1 }}</div>

                <div v-if="block.type === 'TEXT'">
                  <a-textarea v-model:value="block.content" placeholder="请输入文本内容..."
                    :auto-size="{ minRows: 3, maxRows: 10 }" style="width: 100%" />
                </div>
                ...
                <div v-if="block.type === 'IMAGE'">
                  ...
                  <img v-if="block.content" :src="block.content" class="image-preview" alt="图片预览">
                </div>
              ...
              <div v-if="formState.blocks" class="empty-state">
          ...
          <div class="stats-bar">
            ...
            <div class="stat-item">
              <span>总块数</span>
              <span class="stat-value">{{ formState.blocks }}</span>
            </div>
          </div>
...
import { createArticleUsingPost } from '@/api/articleController'

const formState = reactive<API.ArticleCreateRequest>({
})
...
tagOptions.value = useTags((data: string) => {
      return {
        value: data,
        label: data,
      }
